...
```
# I would not consider these two samples to be equal because there is essentially no direct overlap, and the majority of data for each sample is clustered around one specific area, of which are significantly different from each other. There is not much variance, although sample 1 has some apparent outliers.
### Example 2
```{r}
...
ggplot(data = df2, aes(x = samples, #plot data frame 
  ...
  theme(legend.position = "none")
```
# Sample 4 has relatively high variance, with data frames overlapping between the two samples. I would still not consider the two samples equal, although the difference is now less significant than the first two samples. The sample size seems to be too small to formulate accurate conclusions.
### Example 3
```{r}
...
ggplot(data = df3, aes(x = samples, #plot data frame 
  ...
  theme(legend.position = "none")
```
# I would adamantly consider these two samples unequal. There are two distinct areas of clustered data for the samples, with relatlively little variance from those areas. The centers of each sample are significantly and evidently separate, and the sample size is relatively high.
### Example 4
```{r}
...
ggplot(data = df4, aes(x = samples, #plot data frame 
  ...
  theme(legend.position = "none")
```
# These samples may be considered equal. There is high variance among both samples, with a significant amount of overlap between them. The centers of each sample are close, and because the data is not clustered, a conclusion that they are different cannot be soundly made, despite the higher sample sizes. 
# Import data 
...
```
# The data from sample 1 in week 1 and week 3 differ from normal, as their p-values are relatively low. The paired t-test resulted in a p-value of 0.24, which is not low enough to be considered statistically significant. It can be concluded that there is no difference between week 1 and week 3 of the 1xdaily group.
...
```{r}
# breath methane
methane_wk3 <- breath_wkly %>%
  rename_all(tolower) %>%
  filter(study_week == "week1" | study_week == "week3", quantity_compliant == "yes", ch4_mean != 0.0, semester != "Winter2015") %>% 
  na.omit(ch4_mean)
...
```{r}
# acetate
acetate_wk3 <- scfa_long %>%
  filter(study_week == "week1" | study_week == "week3", quantity_compliant == "yes", semester != "Winter2015") %>%
...
pH_wk3 <- pH_wkly %>%
  rename_all(tolower) %>%
  rename(pH_mean = ph_mean, pH_median = ph_median) %>%
  filter(study_week == "week1" | study_week == "week3", quantity_compliant == "yes", semester != "Winter2015") %>%
  na.omit(pH_mean)
...
wk1_methane <- methane_wk3 %>%
  filter(study_week == "week1", 
         supplement_consumed == "BRMPS") 
shapiro.test(wk1_methane$ch4_mean) 
ggplot(wk1_methane, aes(x = ch4_mean)) + geom_histogram()

wk3_methane <- methane_wk3 %>%
  filter(study_week == "week3", 
         supplement_consumed == "BRMPS") 
shapiro.test(wk3_methane$ch4_mean) 
ggplot(wk3_methane, aes(x = ch4_mean)) + geom_histogram()
...
wk1_acetate <- acetate_wk3 %>%
  filter(study_week == "week1", 
         supplement_consumed == "BRMPS") 
shapiro.test(wk1_acetate$acetate_mean) 
ggplot(wk1_acetate, aes(x = acetate_mean)) + geom_histogram()

wk3_acetate <- acetate_wk3 %>%
  filter(study_week == "week3", 
         supplement_consumed == "BRMPS") 
shapiro.test(wk3_acetate$acetate_mean) 
ggplot(wk3_acetate, aes(x = acetate_mean)) + geom_histogram()
...
wk1_propionate <- propionate_wk3 %>%
  filter(study_week == "week1", 
         supplement_consumed == "BRMPS") 
shapiro.test(wk1_propionate$propionate_mean) 
ggplot(wk1_propionate, aes(x = propionate_mean)) + geom_histogram()

wk3_propionate <- propionate_wk3 %>%
  filter(study_week == "week3", 
         supplement_consumed == "BRMPS") 
shapiro.test(wk3_propionate$propionate_mean) 
ggplot(wk3_propionate, aes(x = propionate_mean)) + geom_histogram()
...
ggplot(wk1_pH, aes(x = pH_mean)) + geom_histogram()

wk3_pH <- pH_wk3 %>%
  filter(study_week == "week3", 
         supplement_consumed == "BRMPS") 
shapiro.test(wk3_pH$pH_mean) 
ggplot(wk3_pH, aes(x = pH_mean)) + geom_histogram()
...
methane_new <- inner_join(wk1_methane, wk3_methane,
                          by = c("participant_id", "semester", "supplement_consumed", "quantity_compliant", "frequency")) %>%
  rename(week1 = study_week.x,
         week3 = study_week.y,
         wk1_ch4 = ch4_mean.x,
         wk3_ch4 = ch4_mean.y) %>%
...
wilcox.test(x = methane_new$wk1_ch4, y = methane_new$wk3_ch4, 
       alternative = "less", paired = TRUE, var.equal = FALSE)

# Two-sample Wilcoxon test between week 1 methane and week 3 methane yielded a p-value of 0.001, so there is sufficient evidence to conclude that there was a change in the mean breath methane from week 1 to week 3.
...
```{r}
# acetate
acetate_new <- inner_join(wk1_acetate, wk3_acetate,
                          by = c("participant_id", "semester", "supplement_consumed", "quantity_compliant", "frequency")) %>%
  rename(week1 = study_week.x,
         week3 = study_week.y,
         wk1_acetate = acetate_mean.x,
         wk3_acetate = acetate_mean.y) %>%
  select(-ends_with(".x"), -ends_with(".y"))

wilcox.test(x = acetate_new$wk1_acetate, y = acetate_new$wk3_acetate, 
       alternative = "less", paired = TRUE, var.equal = TRUE)

# Two-sample Wilcoxon test between week 1 and week 3 acetate yielded a p-value of 0.04, so there is enough evidence to conclude that there was a change in mean acetate from week 1 to week 3.
...
```{r}
# propionate
propionate_new <- inner_join(wk1_propionate, wk3_propionate,
                          by = c("participant_id", "semester", "supplement_consumed", "quantity_compliant", "frequency")) %>%
  rename(week1 = study_week.x,
         week3 = study_week.y,
         wk1_propionate = propionate_mean.x,
         wk3_propionate = propionate_mean.y) %>%
  select(-ends_with(".x"), -ends_with(".y"))

wilcox.test(x = propionate_new$wk1_propionate, y = propionate_new$wk3_propionate, 
       alternative = "less", paired = TRUE, var.equal = FALSE)

# Two-sample Wilcoxon test between week 1 and week 3 propionate yielded a p-value of 0.87, so there is not enough evidence to conclude that there was a change in mean propionate from week 1 to week 3.

```

```{r}
# pH
pH_new <- inner_join(wk1_pH, wk3_pH,
                          by = c("participant_id", "semester", "supplement_consumed", "quantity_compliant", "frequency")) %>%
  rename(week1 = study_week.x,
         week3 = study_week.y,
         wk1_pH = pH_mean.x,
...
wilcox.test(x = pH_new$wk1_pH, y = pH_new$wk3_pH, 
       alternative = "greater", paired = TRUE, var.equal = FALSE)

 # Two-sample Wilcoxon test between week 1 and week 3 pH produced a p-value of 0.007, so there is sufficient evidence to conclude that there was a change in mean pH from week 1 to week
```
...
ace_plot <- ggplot(scfa_wide, aes(x = supplement_consumed,
             y = delta_acetate,
             color = supplement_consumed)) +
  geom_hline(yintercept = 0,
             linetype = "dashed") + 
...
save_plot(filename = "Lab5/figures/acetate_plot.pdf",
          plot = ace_plot,
          nrow = 1, ncol = 2,
          base_aspect_ratio = 1.1)

# ANOVA should not be used because some data differ from normal, as indicated by the low p-values resulting from the Shapiro-Wilk normality tests. The Bartlett test also shows a low p-value, indicating differences in variances.
```
...
pro_plot <- ggplot(scfa_wide, aes(x = supplement_consumed,
             y = delta_propionate,
             color = supplement_consumed)) +
  geom_hline(yintercept = 0,
             linetype = "dashed") + 
...
save_plot(filename = "Lab5/figures/propionate_plot.pdf",
          plot = pro_plot,
          nrow = 1, ncol = 2,
          base_aspect_ratio = 1.1)

# ANOVA should not be used because the delta propionate for Arabino supplement does not meet the required sample size minimum. Additionally, not all p-values produced by the Shapiro-Wilk normality tests indicate that the data is normally distributed. The variances are also not homogeneous, as shown in the low p-value produced by the Bartlett test.
