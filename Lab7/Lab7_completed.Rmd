...
# save richness data 
write_delim(richness_df, path = "Lab7/curated_data/richness.txt",
...
# week1 sample size = 97; week3 sample size = 88
richness_df <- richness_df %>%
  spread(study_week, avg_richness) %>%
  na.omit

shapiro.test(richness_df$week1)
ggplot(richness_df, aes(x = week1)) +
  geom_histogram() 
qqnorm(richness_df$week1); qqline(richness_df$week1)
# p-value = 0.8556 and histogram roughly bell-shaped, distribution can be considered normal

shapiro.test(richness_df$week3)
ggplot(richness_df, aes(x = week3)) +
  geom_histogram() 
qqnorm(richness_df$week3); qqline(richness_df$week3)
# p-value = 0.9119 and histogram roughly bell-shaped, distribution can be considered normal

var.test(x = richness_df$week1,
           y = richness_df$week3)
# p-value = 0.4836, not sufficient evidence to conclude that variances are different

# write code for paired t-test here 
  t.test(x = richness_df$week1, y = richness_df$week3, 
...
Since our p-value was less than 0.05, there is sufficient evidence to conclude that average richness did change from week 1 to week 3 in individuals who consumed BRMPS.
...
save_plot(filename = "Lab7/figures/richness_plot.pdf",
          plot = rich2, 
          nrow = 1, ncol = 2,
...
# save Simpson data 
write_delim(simpson_df, path = "Lab7/curated_data/simpson.txt",
            delim = "\t", quote = FALSE, col_names = TRUE)   
```
...
shapiro.test(simpson_df$week1)
ggplot(simpson_df, aes(x = week1)) +
  geom_histogram() 
qqnorm(simpson_df$week1); qqline(simpson_df$week1)
# p-value = 9.318e-10 and histogram skewed, distribution not considered normal

shapiro.test(simpson_df$week3)
ggplot(simpson_df, aes(x = week3)) +
  geom_histogram() 
qqnorm(simpson_df$week3); qqline(simpson_df$week3)
# p-value = 2.082e-11 and histogram skewed, distribution not considered normal

var.test(x = simpson_df$week1, 
         y = simpson_df$week3)
# p-value = 0.833, not sufficient evidence to conclude that variances are different

# write code for paired t-test here 
wilcox.test(x = simpson_df$week1, y = simpson_df$week3, 
         alternative = "two.sided", 
         paired = TRUE, var.equal = TRUE)
# p-value = 0.02852, since our p-value was less than 0.05, there is enough evidence to conclude that Simpson diversity did differ significantly before vs. during supplement consumption.
```
...
save_plot(filename = "Lab7/figures/simpson_plot.pdf",
          plot = simp2, 
          nrow = 1, ncol = 2,
          base_aspect_ratio = 1.1) #upload plot to your GitHub fork 
```
