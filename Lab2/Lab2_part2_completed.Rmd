...
```{r}
num_char <- c(1, 2, 3, "a") #class? all objects become characters
num_logical <- c(1, 2, 3, TRUE) #class? all objects become numerics
char_logical <- c("a", "b", "c", TRUE) #class? all objects become characters
tricky <- c(1, 2, 3, "4") #class? all objects become numerics
# Why do you think it happens? In each case there is an object class being prioritized, and converting all other classes to that class
```
...
```
# in combined logical, only one value is "TRUE" as a chraacter.
...
study_sem <- c("W15", "F15", "W16", "F16", "W17", "F17", "W18", "F18", "W19", "F19")
# Build a vector where the supplements consumed matches the order of the study semesters: 
supplement_sem <- c("RPS", "RMS", "RPS", "Inulin", "Arabinoxylan", "RPS", "RPS", "RMS", "Inulin", "Accessible", "RPS", "HiMaize", "banana", "HiMaize", "RPS", "RMS", "RPS", "RPS", "oat", "rice", "potato")
# What type are each of these? they are all characters
...
round(mean(as.numeric(pH_1)), 2)
# 7.4
pH_2 <- c("8", "7.25", "7.25", "7.75", "7.5", "7.25")
round(mean(as.numeric(pH_2)), 2)
# 7.5
```
