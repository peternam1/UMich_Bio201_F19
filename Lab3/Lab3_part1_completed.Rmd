...
dim() # dimensions (number of rows and number of columns)
colnames() # column titles
rownames() #row titles
glimpse() # sample output of data
...
f18_but_rps <- scfa_indv %>%
  ...
  filter(notes != "frozen >24hrs") %>%
  filter(sample_wt_g > 0.10 & sample_wt_g <= 1.0) %>%
...
scfa_indv_qc <- scfa_indv %>%
  rename(participant_sex = sex, participant_age = age, participant_race = `race/ethnicity`, study_semester = semester, supplement_frequency = frequency, additional_notes = notes) %>% 
  mutate(ht_m = ht_in / 35.274, round(ht_m, digits = 0)) %>%
  mutate(wt_kg = wt_lbs / 2.205, round(wt_kg, digits = 0)) %>%
  filter(sample_wt_g > 0.1 & sample_wt_g <= 1.0) %>%
  filter(additional_notes != "frozen >24hrs") %>%
  mutate(sample_wt_kg = sample_wt_g / 1000,
         acetate_mmol_kg = acetate_mM / sample_wt_kg,
         butyrate_mmol_kg = butyrate_mM / sample_wt_kg,
         propionate_mmol_kg = propionate_mM / sample_wt_kg,
         tot_scfa_mmol_kg = acetate_mmol_kg + butyrate_mmol_kg + propionate_mmol_kg, round(tot_scfa_mmol_kg, digits = 2)) %>%
  select(participant_id, participant_sex, participant_age, participant_race, ht_m, wt_kg, sample_number, sample_id, study_week, study_semester, supplement_consumed, use_data, quantity_compliant, supplement_frequency, sample_wt_kg, tot_scfa_mmol_kg, additional_notes)
...
scfa_wkly_qc <- read_delim(file = "raw_data/SCFA_wkly.txt", 
                        delim = "\t", escape_double = FALSE, trim_ws = TRUE, na=c("NA"),
                        col_types = list()) %>%
  rename(participant_id = Participant_ID, study_week = Study_week, study_semester = Semester, supplement_consumed = Supplement_consumed, quantity_compliant = Quantity_compliant, supplement_frequency = Frequency, acetate_mean_mmol_kg = Acetate_mean,acetate_median_mmol_kg = Acetate_median, butyrate_mean_mmol_kg = Butyrate_mean, butyrate_median_mmol_kg = Butyrate_median, propionate_mean_mmol_kg = Propionate_mean, propionate_median_mmol_kg = Propionate_median)  %>%
  filter(study_week == "week1" | study_week == "week3") %>%
  filter(quantity_compliant == "yes") %>%
  mutate(mean_total_scfa = acetate_mean_mmol_kg + butyrate_mean_mmol_kg + propionate_mean_mmol_kg) %>%
  mutate(median_total_scfa = acetate_median_mmol_kg + butyrate_median_mmol_kg + propionate_median_mmol_kg)
