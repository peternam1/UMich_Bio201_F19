...
scfa_indv_qc <- scfa_indv %>%
  rename(scfa_indv_qc, participant_sex = sex, participant_age = age, participant_race = `race/ethnicity`, study_semester = semester, supplement_frequency = frequency, additional_notes = notes) %>%
  mutate(scfa_indv_qc, participant_id = toupper(participant_id),
    ht_m = ht_in / 35.274,
    wt_kg = wt_lbs / 2.205) %>%
  round(ht_m, digits = 0) %>%
  round(wt_kg, digits = 0) %>%
  filter(sample_wt_g > 0.1 & sample_wt_g <= 1.0) %>%
  filter(additional_notes != "frozen >24hrs") %>%
  mutate(scfa_indv_qc, sample_wt_kg = sample_wt_g / 1000,
         ...
         tot_scfa_mmol_kg = acetate_mmol_kg + butyrate_mmol_kg + propionate_mmol_kg) %>%
  round(tot_scfa_mmol_kg, digits = 2) %>%
