...
# drop median columns
select(scfa_qc, participant_id, study_week, supplement_frequency, study_semester, supplement_consumed, quantity_compliant, acetate_mean_mmol_kg, butyrate_mean_mmol_kg, propionate_mean_mmol_kg, mean_total_scfa) %>%
...
#mutate(scfa_qc, total_scfa = acetate_mmol_kg + butyrate_mmol_kg + propionate_mmol_kg)
```
...
> In the plot where both graphs were generated manually, the axes of each were lined up, but the scales were different for each graph. In the one above, the axes and scales are the same, making it easier to make comparisons and observe distinct patterns. There is also more color on these, making it easier to distinguish between the two different weeks.  
