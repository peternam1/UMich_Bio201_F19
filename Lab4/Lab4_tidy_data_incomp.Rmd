...
group_by(participant_id, study_week) %>%
  summarize(mean_total_scfa = mean(acetate_mmol_kg + butyrate_mmol_kg + propionate_mmol_kg))
...
butyrate_supplement <- butyrate_wide %>%
  ...
  #spread(supplement_consumed, delta_butyrate) #%>% #c
  #na.omit(Accessible, Arabino, BRMPS, BRMPS+Accessible, HiMaize, HiMaize+BRMPS, Inulin, LOODAT, Psyllium, Psyllium+BRMPS, transition_HiMaize) #%>% 
  #mutate(delta_butyrate = week3 - week1)

head(butyrate_supplement)
```
